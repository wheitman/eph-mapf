...
class Network(nn.Module):
    def __init__(
        ...
        self.adv = nn.Linear(self.hidden_dim, config.action_dim * self.num_atoms)
        self.state = nn.Linear(self.hidden_dim, self.num_atoms)
    ...
    def step(self, obs, last_act, pos):
        ...
        if self.selective_comm:
            ...
            adv_val_logits = self.adv(test_hidden).view(
                num_agents, config.action_dim, self.num_atoms
            )
            adv_val_dist = distributions.Categorical(logits=adv_val_logits)
            adv_val = adv_val_dist.sample()

            state_val_logits = self.state(test_hidden).unsqueeze(1)
            state_val_dist = F.softmax(state_val_logits, dim=-1)
            state_val = (state_val_dist * torch.arange(self.num_atoms)).sum(-1, keepdim=True)


            # Tensor to float
            q_val = (state_val.float() + adv_val.float() - adv_val.float().mean(-1, keepdim=True))
            # q_val = (state_val + adv_val - adv_val.mean(-1, keepdim=True))


            actions_mat = (
                torch.ones((num_agents, num_agents), dtype=torch.int) * -1
            )
            actions_mat[test_mask] = q_val.argmax(-1)
            diff_action_mask = actions_mat != actions_mat[
                agent_indexing, agent_indexing
            ].unsqueeze(1)
        ...
        adv_val_logits = self.adv(self.hidden).view(
            num_agents, config.action_dim, self.num_atoms
        )
        adv_val_dist = distributions.Categorical(logits=adv_val_logits)
        adv_val = adv_val_dist.sample()

        state_val_logits = self.state(self.hidden).unsqueeze(1)
        state_val_dist = F.softmax(state_val_logits, dim=-1)
        state_val = (state_val_dist * torch.arange(self.num_atoms)).sum(-1, keepdim=True)

        q_val = (state_val.float() + adv_val.float() - adv_val.float().mean(-1, keepdim=True).float())
        # q_val = state_val + adv_val - adv_val.mean(-1, keepdim=True)
    ...
    def forward(self, obs, last_act, steps, hidden, relative_pos, comm_mask):
        ...
        adv_val_logits = self.adv(hidden).view(
            batch_size, num_agents, config.action_dim, self.num_atoms
        )
        adv_val_dist = distributions.Categorical(logits=adv_val_logits)
        adv_val = adv_val_dist.sample()

        state_val_logits = self.state(hidden).unsqueeze(2)
        state_val_dist = F.softmax(state_val_logits, dim=-1)
        state_val = (state_val_dist * torch.arange(self.num_atoms)).sum(-1, keepdim=True)
